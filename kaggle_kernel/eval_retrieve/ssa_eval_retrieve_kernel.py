"""Buried-fact retrieval at EXTENDED context (32k/64k/128k under YaRN x4).

Tests the document dream: can the model actually FIND a fact buried deep in a
whole long document (not just stay coherent over it)? Plants a passkey at depths
0.1/0.5/0.9, asks for it, checks the generation. Compares full attention vs
sparse + 8k-adapter. Earlier passkey NIAH maxed at 32k and saturated; this pushes
to 128k under window-stretching, where retrieval should actually get hard.
"""
...
FILLER = ("The sky was clear and the grass was green. People walked through the "
          "park and chatted about the weather and their weekend plans. ")
...
for name, sp, ad in [("FULL", False, False), ("SPARSE + 8k-adapter", True, True)]:
    print(f"\n=== RETRIEVAL: {name} (YaRN x4) ===", flush=True)
    ...
    for L in LENGTHS:
        try:
            ...
            print(f"{L:>7} | {'OOM/ERR: ' + str(ex)[:40]}", flush=True)
            gc.collect(); torch.cuda.empty_cache()
